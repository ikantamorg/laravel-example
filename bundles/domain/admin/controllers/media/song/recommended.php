...
class Admin_Media_Song_Recommended_Controller extends Rest_Controller
{
	public $resource_type = 'songs';

	public $fields = ['type', 'resource_id'];
	public $view_base = 'admin::media.songs.recommended.';
	public $base_uri = 'admin/media/song/recommended/';

	protected $_listing = [];

	protected function existing_recommended_ids()
	{
		$rows = DB::table('core_recommended')->where_type($this->resource_type)->select('resource_id')->get();
	...
	public function listing()
	{
		...
		$recommended_ids = $this->existing_recommended_ids();

		return $this->_listing = $recommended_ids ? 
								  Song::where_in('id', $recommended_ids)->get()
								: [];
	}
	...
	protected function form()
	{
		$form = Hybrid\Form::make(function ($f) {
			...
			$f->fieldset('Recommended Songs', function ($fs) {
				$fs->control('select', 'Songs', function ($c) {
					$c->name = $this->resource_type.'[]';
					
					$options = [];
					foreach(Song::all() as $a)
						$options[$a->id] = $a->name;
					$c->options = $options;
	...
	public function put_update()
	{
		...
		$existing_ids = $this->existing_recommended_ids();

		$new_ids = array_diff($input_ids, $existing_ids);
		$removed_ids = array_diff($existing_ids, $input_ids);

		if($removed_ids)
			DB::table('core_recommended')->where_type($this->resource_type)->where_in('resource_id', $removed_ids)->delete();

		foreach($new_ids as $id)
			DB::table('core_recommended')->insert([
				'type' => $this->resource_type, 'resource_id' => $id,
				'created_at' => new DateTime, 'updated_at' => new DateTime
			]);

		return Redirect::to($this->base_uri)->with('flash.success', $this->resource_type . 'recommended succesfully');
	}
}