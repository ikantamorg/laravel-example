...
class Admin_Media_Video_Recommended_Controller extends Rest_Controller
{
	public $resource_type = 'videos';

	public $fields = ['type', 'resource_id'];
	public $view_base = 'admin::media.videos.recommended.';
	public $base_uri = 'admin/media/video/recommended/';
	...
	public function listing()
	{
		...
		return $this->_listing = $recommended_ids ? 
								  Video::where_in('id', $recommended_ids)->get()
								: [];
	}
	...
	protected function form()
	{
		$form = Hybrid\Form::make(function ($f) {
			...
			$f->fieldset('Recommended Videos', function ($fs) {
				$fs->control('select', 'Videos', function ($c) {
					$c->name = $this->resource_type.'[]';
					
					$options = [];
					foreach(Video::all() as $a)
						$options[$a->id] = $a->name;
					$c->options = $options;
